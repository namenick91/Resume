...
\section{ОПЫТ РАБОТЫ}
  \resumeSubHeadingListStart
    ...
    \resumeSubheading
      ...
      \resumeItemListStart
        \resumeItem{Проведение сравнительного исследования VLM OCR-систем на различных типах документов (6 систем $\times$ 5 сценариев): инференс на GPU A100, метрики под каждый сценарий (CER/WER, PER, CDM).}
        \resumeItem{Разработка FastAPI-сервиса для парсинга документов поверх PaddleOCR-VL-1.5: vLLM инференс, Docker-развёртывание; результат OCR преобразуется в структурированный граф документа. Micro-CER сервиса -- 3.7\%}
        \resumeItem{Разработка бенчмарка для выбора VLM, извлекающей данные из разнотипных графиков в табличный вид (chart-to-table), с целью дальнейшего внедрения в сервис: 11 моделей, 5 датасетов, 8 метрик с bootstrap-CI.}
        \resumeItem{Рецензирование научных статей по анализу документов методами DL.}
      \resumeItemListEnd
    ...
    \resumeSubheading
      ...
      \resumeItemListStart
        \resumeItem{Разработка вопросно-ответной RAG-системы для образовательной платформы NOMOTEX: ETL разнородных источников, гибридный поиск (BGE-M3) и хранение: Qdrant + PostgreSQL, диалоговая память, ссылки на источники в ответах модели.}
        \resumeItem{Оценка качества (RAGAS) и выбор конфигурации системы. Сейчас -- внедрение на платформу.}
      \resumeItemListEnd
...
\section{НАУЧНАЯ РАБОТА}
    \resumeSubHeadingListStart
      \resumeProjectHeading
          ...
          \resumeItemListStart
          \resumeItem{Расширил эмбеддинг Informer двухслойным сверточным блоком, добавил автоматическую декомпозицию ряда из Autoformer и заменил ProbSparse на механизм внимания FAVOR+ (Performer), работающий за линейное время.}
          \resumeItem{На длинных горизонтах получил меньший MSE на стандартных бенчмарках (ETT, ECL и др.).}
            \resumeItem{Препринт статьи готов, идёт работа над публикацией в журнале Q2-Q3; код и препринт: \href{https://github.com/namenick91/Convformer}{\myuline{GitHub}}.}
          \resumeItemListEnd
...
\section{НАВЫКИ}
 \begin{itemize}[leftmargin=0in, label={}]
    \small{\item{
     \textbf{Инструменты:}{ Python, SQL, NumPy, SciPy, pandas, FastAPI, Streamlit, \LaTeX}\vspace{2pt} \\
     \textbf{ML / DL:}{ PyTorch, scikit-learn, CatBoost, Hugging Face Transformers, RAG; оценка и бенчмаркинг LLM/VLM, прогнозирование временных рядов}\vspace{2pt} \\
